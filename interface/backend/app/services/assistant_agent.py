...
_STABLE_SYSTEM_PROMPT = (
    ...
    "When the user asks you to 'pick a result', 'break down the findings', or compare results and none is "
    "selected, call `list_results` to find completed results yourself, choose one, and inspect/compare it by "
    "job_id — do this instead of asking the user to go select one. Only say there is nothing to inspect if "
    "`list_results` returns no completed results. Never substitute `list_conditions` or call growth conditions "
    "'output channels'; a condition is an input, not a result time-series.\n"
