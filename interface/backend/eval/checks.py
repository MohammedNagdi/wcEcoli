...
def check_faithfulness(content: str, executed_tools: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    conceptual answer has nothing to ground against)."""
    if not executed_tools:
        return {"check": "faithfulness", "passed": True, "detail": "skipped (no tool output)"}
    haystack = json.dumps([t.get("result") for t in executed_tools], default=str)
    candidates = set(_NUM_RE.findall(content)) | set(_ID_RE.findall(content))
    unsupported = [
        tok for tok in candidates
        if tok not in _FAITHFULNESS_WHITELIST and tok not in haystack
    ]
    return {
        "check": "faithfulness",
